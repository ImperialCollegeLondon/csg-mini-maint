...
=head2 B<maint_initlog(string:scriptname [, scalar:loglevel=Maint::Log::LOG_INFO])>
...
Returns scalar, true if OK, otherwise false.
...
=head2 B<maint_loglevel([scalar:loglevel])>
...
sub maint_loglevel
{
	...
	{
		$loglevel = LOG_INFO;
		maint_log(LOG_ERR, "Maint::Log::maint_loglevel() - tried to set invalid level [$p]");
		return $loglevel;
	}
	$loglevel = $p;
	return $loglevel;
...
=head2 B<maint_logperline([bool:logperline])>

Returns current logperline. If parameter supplied, sets the new logperline to that.
...
=head2 B<maint_colouriselog([bool:colourise])>

Returns current colourise. If parameter supplied, sets the new colourise to that.

If set, then each log message will have ANSI colour escapes embedded.

If unset, each log message will be prefixed with DEBUG:, INFO:, WARN: or ERROR:
...
=head2 B<maint_parsablelog([bool:machineread])>

Returns current machineread. If parameter supplied, sets the new machineread to that.

If set, then each log message will be set to no colour, one per line and be of the
form:
...
=head2 B<maint_tracemode([stringl:tracemode])>

Returns current trace mode. If parameter supplied, sets the new trace mode to that.
...
=head2 B<maint_isexitforced([scalar:log_exit_forced])>

Intended only for the END handler in CSG.pm - will set this flag if log_error
signalled an exit so that CSG.pm's END handler won't whine.
...
{
	my $line = shift;
	print _formatforscreen(LOG_INFO, $line) unless maint_loglevel() < LOG_INFO;
	$line =~ s/%/\%/g;
	_lsyslog(LOG_INFO, $line) if $lsyslogok;
}

=head2 B<maint_log(const:level, string:message)>
...
sub maint_log ($$)
{
	...
	# Only print to screen if the loglevel is geq the level of the
	# message
	return 1 unless maint_loglevel() >= $level;
	print _formatforscreen($level, $line);

	# If this is an error, handle death gracefully.
	if ($level == LOG_ERR) {
...
{
	my ($level, $msg) = @_;
	unless (exists $logstr{$level} && exists $logstr{$level})
	{
		return $msg;
	}
	my $result      = '';
...
{
	unless (maint_logperline() || maint_parsablelog() || maint_loglevel() < LOG_INFO)
	{
		print "\r\n";
	}
}
